...
- Les règles ne sont pas dictées par l'OMC, mais négociées directement entre États membres (en continu à Genève et, en principe, tous les deux ans lors de grandes conférences ministérielles), dans le cadre d'accords commerciaux multilatéraux.
  ...
  Elles concernent:
	...
	- les mesures sanitaires et phytosanitaires (SPS), etc.
	  
	  Ces accords constituent l'architecture réglementaire de la mondialisation.
	- Le principe de libre-échange, essence de l'OMC, puise sa source dans le faisceau très large d'arguments issus du courant libéral.
	- Le « laisser-faire » assure la libre entreprise et la meilleure division du travail possible (A. Smith),
	- le < laisser-passer » conduit à la spécialisation des acteurs selon leurs avantages absolus (A. Smith) ou comparatifs (D. Ricardo),
	- la compétition incite à l'innovation et au progrès technique (J.A. Schumpeter),
	- le marché libéré des entraves protectionnistes devient auto-régulateur (J.-B. Say) et permet la détermination d'un prix d'équilibre (L. Walras) correspondant à l'optimum* (V. Pareto).
	- Le respect de la propriété privée est le meilleur gage d'efficacité économique (L. von Mises), les acteurs les moins efficaces étant contraints à la disparition (F. Hayek).
	- La mobilité des facteurs de production assure la convergence économique < absolue » ou < conditionnelle » (R. M. Solow), etc.
- 1. ouverture d'une < demande d'information » qui correspond à une tentative de médiation et de conciliation à l'amiable (environ un tiers des différends est réglé à ce stade);
- 2. dépôt de plainte et constitution d'un panel d'experts (appelé << groupe spécial ») sur le sujet ;
- 3. remise du rapport du groupe spécial aux parties, puis aux juges de l'ORD;
- 4. jugement rendu par l'ORD;
- 5. dépôt possible d'un appel qui provoque la production d'un rapport d'appel remis à l'ORD et du jugement définitif rendu par l'ORD;
- 6. mise en place d'éventuelles sanctions (appelées < rétorsions commerciales ») en cas de non-application du jugement rendu par l'une ou l'autre des parties.
-
- Les secteurs issus de l'Uruguay Round font l'objet de négociations permanentes à Genève depuis 1995 et constituent ce que l'on appelle l'< agenda incorporé ». Qui regroupe :
...
- Le texte de GATT 1994 n'est plus négociable ni amendable. En revanche, il est complété par des accords commerciaux qui lui sont liés.
  ...
  Citons:
- l'Accord sur « l'évaluation en douane » qui définit les méthodes d'estimation et de calcul de la valeur en
  
  douane des marchandises ainsi qu'un bon nombre de pratiques douanières;
- l'Accord sur < les licences d'importation » qui définit les modalités de délivrance de ces documents douaniers;
  
  l'Accord sur « l'accès aux marchés publics » qui définit les modalités d'accès des marchés publics nationaux pour les entreprises étrangères. Cet accord avait été négocié lors du Tokyo Round, où seuls douze pays l'avaient signé.
-
- L'Accord sur « l'inspection avant expédition » qui définit les conditions de contrôle des marchandises
  
  exportées; L'Accord sur les « obstacles techniques au commerce » (OTC) qui définit des règles pour encadrer les réglementations techniques appliquées au commerce; - L'Accord sur les < subventions et les mesures compensatoires » qui définit les modalités précises de cette pratique.
- ### le commerce des services
- L'AGCS fournit un cadre juridique nouveau et adapté définissant:
- les < secteurs visés »,
- les < modes de fourniture »,
...
- ## Les Modes de Fourniture des Services (AGCS)
  L'AGCS définit quatre manières dont un service peut être échangé au niveau international, selon la localisation du prestataire et du consommateur.
- ### 1. Mode 1 : La prestation transfrontalière
  * **Principe :** Le service traverse la frontière, mais ni le prestataire ni le client ne se déplacent physiquement.
  * **Mécanisme :** Le service est fourni depuis le territoire d'un pays vers le territoire d'un autre pays.
  * **Exemple :** L'expertise d'un avocat envoyée par courriel ou par courrier à un client situé à l'étranger.
- ### 2. Mode 2 : La consommation à l'étranger
  * **Principe :** C'est le **consommateur** qui se déplace.
  * **Mécanisme :** Le service est fourni sur le territoire du prestataire à un client venant d'un autre pays.
  * **Exemple :** Un touriste qui voyage pour consommer des services hôteliers ou un patient qui se rend à l'étranger pour des soins médicaux.
- ### 3. Mode 3 : La présence commerciale à l'étranger
  * **Principe :** C'est l'**entreprise** qui s'installe durablement dans le pays du client.
  * **Mécanisme :** Le service est fourni via une structure physique (filiale ou succursale) que le prestataire contrôle directement dans le pays de destination.
  * **Caractéristique :** Il y a une implantation directe sur le marché local.
- ### 4. Mode 4 : La présence de personnes physiques
  * **Principe :** Ce sont des **individus** (travailleurs) qui se déplacent temporairement.
  * **Mécanisme :** Le prestataire fournit le service en envoyant des personnes physiques chez le client.
  * **Distinction :** Contrairement au Mode 3, il n'y a **pas d'implantation** de filiale ou de succursale permanente.
  * **Exemple :** Un consultant ou un ingénieur qui se rend physiquement dans les bureaux d'un client à l'étranger pour une mission spécifique.
- ### Tableau Récapitulatif
  | Mode | Désignation | Qui ou quoi se déplace ? |
  |---|---|---|
  | **Mode 1** | Transfrontalier | Le **service** uniquement |
  | **Mode 2** | Consommation à l'étranger | Le **client** |
  | **Mode 3** | Présence commerciale | L'**entreprise** (filiale) |
  | **Mode 4** | Personnes physiques | L'**employé** (consultant) |
- Les règles de l'AGCS
	- Les « règles générales » de l'AGCS sont calquées sur celles du GATT 1994. On y retrouve les clauses NPF, de réciprocité, de transparence, etc. Toutefois, la notion de < tarif douanier » a été remplacée par celle de < règles d'accès au marché », définissant l'ensemble des restrictions autorisées. Les négociations en cours autour de l'AGCS portent les subventions, les régimes de sauvegarde et les marchés publics.
	- Les < règles spécifiques » sont définies dans des annexes sectorielles issues de négociations ne portant que sur quelques secteurs particuliers: télécommunications (accord conclu en 1997), mouvement des personnes physiques, services financiers (accord de 1997), transports maritime et aérien.
- Les droits de propriété intellectuelle liés au commerce (TRIPs)
	- L'Accord sur les aspects des droits de propriété intellectuelle liés au commerce (ADPIC, ou TRIPs en anglais) intègre et étend la Convention de Paris (sur la protection de la propriété industrielle) et celle de Berne (sur la protection des œuvres littéraires et artistiques). L'objet de l'ADPIC est de poser des normes de protection minimale des droits de propriété qui doivent être mises en place par la législation nationale de chaque pays membre.
	- L'accord prévoit des mesures juridiques nationales et internationales spécifiques mises à la disposition des détenteurs des droits de propriété. En cas de fraude dans un pays, c'est l'État qui est considéré comme responsable par l'instauration d'un mécanisme renforcé au sein de l'ORD.
  ...
  collapsed:: true
	- ## LʼAccord sur les technologies de lʼinformation (ATI)
		- Signé par 29 pays membres à la conférence ministérielle de Singapour (1996), cet accord regroupe désormais 82
		  ...
		  Lʼobjectif de cet accord plurilatéral est lʼélimination complète des droits de douane sur les produits des
		  technologies de lʼinformation : 201 produits au total dont le commerce annuel est estimé à plus de 1300 milliards
		  de dollars.
	- LʼAccord sur les marchés publics (AMP)
		- Signé en 1994, révisé en 2014, cet accord compte actuellement 20 Parties représentant 48 membres de lʼOMC (UE
		  et ses 27 Etats membres ainsi que le Royaume-Uni comptent comme un seule Partie) et 35 pays en qualité
		  dʼobservateurs (dont 11 ont engagé e processus dʼaccession à lʼAccord).
		  Lʼobjectif de lʼAccord est lʼouverture mutuelle des marchés publics entre les Parties. Les principes et règles de
		  procédures de lʼAMP ne sʼapplique pas automatiquement à toutes les activités de passation de marché. Les Parties
